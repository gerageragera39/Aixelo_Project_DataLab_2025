...
df.to_csv("120_fingerprints_with_energy.csv")
...
df['MOF'] = df.index.map(extract_base_mof_name)

fingerprint_cols = [col for col in df.columns if col not in ['energy', 'MOF']]
agg_dict = {col: 'first' for col in fingerprint_cols}
agg_dict['energy'] = 'mean'

df_grouped = df.groupby('MOF').agg(agg_dict)
df_grouped.index.name = 'MOF'
